...
import spark_examples.Utilities._
  ...
  def main(args: Array[String]) {
    ...
    statuses.foreachRDD((rdd, time) => {
      ...
      if (rdd.count() > 0) {
        ...
        repartitionedRDD.saveAsTextFile("out/Tweets_" + time.milliseconds.toString)
    ...
    ssc.checkpoint("/Users/dzmitrydubarau/Documents/Learning/OngoingJavaLearning/ScalaTutorial/checkpoint")
    ssc.start()
